  ...
  async fetch(request: Request, env: Env) {
    ...
    if (url.pathname.startsWith('/api/')) {
      ...
      try {
        ...
        // 配置相关API
        ...
        } else if (path === 'configs/batch' && method === 'PUT') {
          // 批量更新配置
          const configs = (await request.json()) as Record<string, string>;
          ...
          // 批量更新配置
          const result = await api.batchSetConfig(configs);
          return Response.json({ success: result });
